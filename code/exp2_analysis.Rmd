...
bootstrap_diff = function(x1, x2) {
  sample1 = sample(x1, length(x1), replace=T)
  sample2 = sample(x2, length(x2), replace=T)
...
diffs = replicate(1000, bootstrap_diff(xtop$md/xtop$fd, xbot$md/xbot$fd))
mean(diffs) # -0.137364
sort(diffs)[25] # -0.1460871
sort(diffs)[975] # -0.1289108
...
sd(xbot$gd) # 0.116323
diffs = replicate(1000, bootstrap_diff(xbot$gd, xtop$gd))
mean(diffs) # 0.5979193
sort(diffs)[25] # 0.5819568
sort(diffs)[975] # 0.6151206
...
diffs = replicate(1000, bootstrap_diff(1-xtop$dpsg, 1-xbot$dpsg))
mean(diffs) # 0.03404985
sort(diffs)[25] # 0.0295291
sort(diffs)[975] # 0.03908275
```
