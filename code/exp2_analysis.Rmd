...
## fig S3 unpacked for top and bottom acc gropus
```{r}
data.subgroup = data.main %>%
  filter(subject %in% top_half) %>%
  mutate(pt=planningTime/1000) %>%
  group_by(subject) %>%
  mutate(meanpt=mean(pt),
...
p1.plotdata = data.subgroup %>%
  filter(trialAdvType=='NT') %>%
  mutate(totalAdv=factor(abs(totalAdv))) %>%
  mutate(level = ifelse(level=='base', level, ifelse(goalLoc==1, 'subgoal: upper goal', 'subgoal: lower goal'))) %>%
  mutate(level = str_wrap(level, width=8))

p1.path = p1.plotdata %>%
  group_by(subject, totalAdv, level) %>%
  summarize(upperPathTaken=mean(pathTaken=='upper')) %>%
  mutate(upperPathTakenProbit=qnorm(upperPathTaken)) %>%
  mutate(upperPathTakenProbit = ifelse(upperPathTakenProbit > 3, 3,
                                       upperPathTakenProbit)) %>%
  mutate(upperPathTakenProbit = ifelse(upperPathTakenProbit < -3, -3,
                                       upperPathTakenProbit)) %>%
  ggplot(., mapping=aes(x=totalAdv, y=upperPathTakenProbit)) +
  facet_grid(.~level) +
  stat_summary(fun.data = "mean_cl_boot",
               geom = "pointrange",
               position = position_dodge(width=0.2),
...
p1.rt = p1.plotdata %>%
  group_by(subject, level, totalAdv) %>%
  summarize(medianzpt=median(zpt),
            popmeanpt=mean(popmeanpt),
            popstdpt=mean(popstdpt)) %>%
  mutate(projectedmedianzpt=medianzpt*popstdpt+popmeanpt) %>%
  ggplot(., mapping=aes(x=totalAdv, y=projectedmedianzpt)) +
  facet_grid(.~level) +
  stat_summary(fun.data = "mean_cl_boot",
...
```{r}
p2.plotdata = data.subgroup %>%
  filter(trialAdvType=='IA') %>%
  mutate(totalAdv=factor(abs(totalAdv)),
         myopicQuantAdv=factor(abs(myopicQuantAdv), levels=c(1,2))) %>%
  mutate(trialAdvType=as.character(trialAdvType)) %>%
  mutate(trialAdvType=ifelse(trialAdvType=='IA', paste(trialAdvType,myopicQuantAdv,sep='-'),
                             trialAdvType)) %>%
  mutate(trialAdvType=factor(trialAdvType, levels=c('IA-1','IA-2'))) %>%
  mutate(myopicGoalSameSide = ((myopicAdvPath=='upper')&(goalLoc==1)) |
                              ((myopicAdvPath=='lower')&(goalLoc==9))) %>%
  mutate(level=ifelse(level=='base', level, ifelse(myopicGoalSameSide, 
                                                   'subgoal: goal same with mAdv',
                                                   'subgoal: goal diff from mAdv'))) %>%
  mutate(level=factor(level, levels=c('base','subgoal: goal diff from mAdv','subgoal: goal same with mAdv'))) %>%
  mutate(level = str_wrap(level, width=12))
...
p2.path = p2.plotdata %>%
  ...
  mutate(pathFavorsMyopicAdvProbit = ifelse(pathFavorsMyopicAdvProbit < -3, -3,  # cap infs
                                            pathFavorsMyopicAdvProbit)) %>%
  ggplot(., mapping=aes(x=totalAdv, y=pathFavorsMyopicAdvProbit, color=trialAdvType)) +
  facet_grid(.~level) +
  stat_summary(fun.data = "mean_cl_boot",
               geom = "pointrange",
               position = position_dodge(width=0.2),
               ...
               fill = 'white',
               shape = 22) +
  scale_color_manual(values=c(redblue[4], redblue[5]), name='') +
  labs(x='Total Path Difference',
       y='Path Favors Myopic Advantage\n(probit space)') +
  coord_cartesian(ylim=c(-1.5, 3)) +
  theme(legend.position='none')
...
p2.rt = p2.plotdata %>%
  ...
  summarize(medianzpt=median(zpt),
            ...
            popstdpt=mean(popstdpt)) %>%
  mutate(projectedmedianzpt=medianzpt*popstdpt+popmeanpt) %>%
  ggplot(., mapping=aes(x=totalAdv, y=projectedmedianzpt, color=trialAdvType)) +
  facet_grid(.~level) +
  stat_summary(fun.data = "mean_cl_boot",
               geom = "pointrange",
               position = position_dodge(width=0.2),
...
```{r}
p3.plotdata = data.subgroup %>%
  filter(totalAdv!=0) %>%
  mutate(absTotalAdv=factor(abs(totalAdv))) %>%
  mutate(optimPathGoalConsistency = ((totalAdvPath=='upper')&(goalLoc==1)) |
                                    ((totalAdvPath=='lower')&(goalLoc==9))) %>%
  mutate(level=ifelse(level=='base', level, ifelse(optimPathGoalConsistency, 
                                                   'subgoal: goal same with optimal path',
                                                   'subgoal: goal diff from optimal path'))) %>%
  mutate(level=factor(level, levels=c('base','subgoal: goal diff from optimal path','subgoal: goal same with optimal path'))) %>%
  mutate(level = str_wrap(level, width=15))
...
p3.rt = p3.plotdata %>%
  ...
  summarize(medianzpt=median(zpt),
            ...
            popstdpt=mean(popstdpt)) %>%
  mutate(projectedmedianzpt=medianzpt*popstdpt+popmeanpt) %>%
  ggplot(., mapping=aes(x=absTotalAdv, y=projectedmedianzpt, 
                        fill=trialAdvType, shape=trialAdvType)) +
  facet_grid(.~level) +
  stat_summary(fun.data = "mean_cl_boot",
               geom = "pointrange",
...
p1.path + p2.path + p3.path + p1.rt + p2.rt + p3.rt +
  plot_annotation() +
  plot_layout(ncol=3, widths = c(1,1.2,2.5)) &
  theme(axis.title = element_text(size=11))
 
# ggsave(paste(plotout,'exp2_top_unpacked.pdf', sep=''), bg='transparent')
# ggsave(paste(plotout,'exp2_bottom_unpacked.pdf', sep=''), bg='transparent')
```
...
p.ia.main = data.ia %>%
  ...
  mutate(trialType = factor(trialType, levels = c('subgoal (goal on myopic side)','subgoal (goal on future side)','base'))) %>%
  ...
  mutate(pathFavorsMyopicAdvProbit = ifelse(pathFavorsMyopicAdvProbit < -3, -3,  # cap infs
                                            pathFavorsMyopicAdvProbit)) %>%
  ungroup() %>%
  mutate(jitter=ifelse(trialType=='base', 0, ifelse(trialType=='subgoal (goal on myopic side)', -0, 0))) %>%
  ggplot(data=., mapping=aes(x=relativeAdv+jitter, y=pathFavorsMyopicAdvProbit,
                             color=trialType, fill=trialType, shape=trialType)) +
  ...
  stat_summary(fun.data = "mean_cl_boot",
               ...
               color = 'black',
               size = .5) +
  scale_fill_manual(values=c('black','black','white'), name='Trial Type') +
  scale_shape_manual(values=c(23,22,21), name='Trial Type') +
...
p.ia.bot = data.ia %>%
  ...
  mutate(trialType = ifelse(level=='base', 'base',
                     ifelse(myopicGoalSameSide, 'subgoal (goal on myopic side)', 'subgoal (goal on future side)'))) %>%
  mutate(trialType = factor(trialType, levels = c('base','subgoal (goal on myopic side)','subgoal (goal on future side)'))) %>%
  filter(! subject %in% top_half, level!='base') %>%
  group_by(subject, relativeAdv, trialType) %>%
  summarize(pathFavorsMyopicAdv = mean(pathStart==myopicAdvPath)) %>%
  mutate(pathFavorsMyopicAdvProbit = qnorm(pathFavorsMyopicAdv)) %>%
  mutate(pathFavorsMyopicAdvProbit = ifelse(pathFavorsMyopicAdvProbit > 3, 3,  # cap infs
  ...
  stat_summary(fun.data = "mean_cl_boot",
               ...
               fill = 'black',
               size = .5) +
  annotate(geom='text', x=-2, y=2, label='Less accurate\nparticipants', color='black', size=4, hjust=0) +
  scale_shape_manual(values=c(23,22), name='Trial Type') +
  labs(x = 'Myopic/Future Relative Advantage',
       y = 'Path Favors Myopic Advantage\n(probit space)') +
  coord_cartesian(ylim=c(-2.7,2.7),
                  xlim=c(-2.3,2.3)) +
...
```{r}
# additional fits on random-splitting the participant group 1000 times
# to obtain baseline parameter estimates for comparison of top/bottom group fits
setwd('~/github/weighted-goal-satisfaction-revision/code/')

bottom_half = unique(data.fit %>% filter(! subject %in% top_half) %>% pull(subject))
all_subjects = unique(data.fit$subject)

# fit all N=50 for top and bottom groups
# cv_folds = as.data.frame(matrix(nrow=50, ncol=1))
# cv_folds[,1] = bottom_half
# cv_folds_train = cv_folds
# cv_folds_test = cv_folds

# fit on 1000 random splits
# cv_folds = as.data.frame(matrix(nrow=100, ncol=1000))
# for (s in 1:1000) {
#     cv_folds[, s] = sample(all_subjects, 100) # shuffle participant list
# }
# saveRDS(cv_folds, 'exp2_cv_fold_baseline_1000_shuffled_participants_list.RData')
cv_folds_first_group = cv_folds[1:50,]
cv_folds_second_group = cv_folds[51:100,]
```

``` {r}
cv_folds_train = cv_folds_second_group
cv_folds_test = cv_folds_second_group

cross_val_paired_boot = function(i, model_spec, data, nfit) {
    # performs one round of monte-carlo cross validation
    train_subjects = cv_folds_train[,i]
    test_subjects = cv_folds_test[,i]
    train_data = data %>% filter(subject %in% train_subjects)
    test_data = data %>% filter(subject %in% test_subjects)
    best_fit = find_best_fit(model_spec,
                             train_data=train_data,
                             test_data=test_data,
...
spec = c('t0','a','md','fd','sz','sd','dpsg','gd')
model_out = paste('./exp2_diffusion_revision/fixed_cv/exp2_fits_baseline_group2_',
                  paste(spec, collapse='_'),
                  '.RData', sep='')
if(! file.exists(model_out)) {
    cat(paste('fitting and saving to ',model_out,'......\n',sep=''))
    t0 = Sys.time()
    fits = mclapply(1:1000, function(i) {cross_val_paired_boot(i, spec, data.fit, nfit=10)},
                    mc.cores=2)
    t1 = Sys.time()
    cat(t1-t0)
    saveRDS(fits, model_out)
...
p.model.comp.full = ggplot() +
  ...
  geom_text(mapping=aes(x=0.2, y=0, label='baseline sNLL = 4013.10 (SD=114.45) '),
            hjust=1, va='right', angle=90, size=3, color='black') +
  ...
  theme(legend.position=c(0.62, 0.1),
...
paired_fits = list.files(path='./exp2_diffusion_revision/fixed_cv', 
                  pattern='exp2_fits_tophalf*', full.names=T)
xtop = do.call(bind_rows, lapply(paired_fits, read_fit)) %>%
  mutate_at(c('t0','a','md','fd','sz','sd','dpsg','gd',
              'objective','test_objective'), as.numeric) %>%
  mutate(model=str_replace(model, 't0_a_md_fd_sz_sd', 'basemodel')) %>%
  select(md, fd, gd, dpsg) %>% 
  mutate(group='tophalf')

paired_fits = list.files(path='./exp2_diffusion_revision/fixed_cv', 
                         pattern='exp2_fits_bottomhalf*', full.names=T)
xbot = do.call(bind_rows, lapply(paired_fits, read_fit)) %>%
  mutate_at(c('t0','a','md','fd','sz','sd','dpsg','gd',
              'objective','test_objective'), as.numeric) %>%
  mutate(model=str_replace(model, 't0_a_md_fd_sz_sd', 'basemodel')) %>%
  select(md, fd, gd, dpsg) %>% 
  mutate(group='bottomhalf')

# ratio comparison
mean(xtop$md/xtop$fd) # 1.166132
sd(xtop$md/xtop$fd) # 0.02463593
mean(xbot$md/xbot$fd) # 1.303483
sd(xbot$md/xbot$fd) # 0.05758869

# goal weight comparison
mean(xtop$gd) # 0.08209403
sd(xtop$gd) # 0.03376332
mean(xbot$gd) # 0.6799137
sd(xbot$gd) # 0.116323

# degradation comparison
mean(1-xtop$dpsg) # 0.308898
sd(1-xtop$dpsg) # 0.0194196
mean(1-xbot$dpsg) # 0.274907
sd(1-xbot$dpsg) # 0.02646905
```
...
xbase = xtop %>%
  ...
  mutate(group=ifelse(group=='tophalf', 'More\nAccurate', 'Less\nAccurate')) %>%
  mutate(param=factor(param, levels=c('Start','Goal')),
         group=factor(group, levels=c('More\nAccurate','Less\nAccurate'))) %>%
  mutate(level='base')
...
xsubgoal = xtop %>%
  ...
  mutate(param=ifelse(param=='mdsg', 'Start', 
                      ifelse(param=='fdsg', 'Subgoal', 'Final Goal'))) %>%
  mutate(group=ifelse(group=='tophalf', 'More\nAccurate', 'Less\nAccurate')) %>%
  mutate(param=factor(param, levels=c('Start','Subgoal','Final Goal')),
         group=factor(group, levels=c('More\nAccurate','Less\nAccurate'))) %>%
  mutate(level='subgoal')

pleft = ggplot() +
  stat_summary(data=xbase, 
               mapping=aes(x=group, y=value, group=param, color=param, fill=param),
               fun='mean',
               geom='bar',
               width=0.8,
               position=position_dodge(width=.9)) +
  stat_summary(data=xbase,
               mapping=aes(x=group, y=value, group=param),
               fun.data='mean_cl_boot',
               geom='errorbar',
               position=position_dodge(width=.9)) +
  scale_color_manual(values=c('black', 'black')) +
  scale_fill_manual(values=c('white', 'gray')) +
  labs(y='Weight', x='                                               Participant Group', fill='', color='') +
  scale_y_continuous(limits=c(0,2.3), breaks=c(0,1,2)) +
  geom_text(mapping=aes(x=1, y=2.3, label='Base Task'),
...
pright = ggplot() +
  ...
  scale_fill_manual(values=c('white', 'gray', 'gray30')) +
  labs(y='Weight', x='', fill='', color='') +
  scale_y_continuous(limits=c(0,2.3), breaks=c(0,1,2)) +
  geom_text(mapping=aes(x=1, y=2.3, label='Complex Task'),
            hjust=0, size=4, color='black') +
...
# top/bottom group param comparison
```{r}
paired_fits = list.files(path='./exp2_diffusion_revision/fixed_cv', 
                  pattern='exp2_fits_fitall_tophalf*', full.names=T)
xtop = do.call(bind_rows, lapply(paired_fits, read_fit)) %>%
  mutate_at(c('t0','a','md','fd','sz','sd','dpsg','gd',
              'objective','test_objective'), as.numeric) %>%
  mutate(model=str_replace(model, 't0_a_md_fd_sz_sd', 'basemodel'))
...
paired_fits = list.files(path='./exp2_diffusion_revision/fixed_cv', 
                         pattern='exp2_fits_fitall_bottomhalf*', full.names=T)
xbot = do.call(bind_rows, lapply(paired_fits, read_fit)) %>%
  mutate_at(c('t0','a','md','fd','sz','sd','dpsg','gd',
              'objective','test_objective'), as.numeric) %>%
  mutate(model=str_replace(model, 't0_a_md_fd_sz_sd', 'basemodel'))
...
paired_fits = list.files(path='./exp2_diffusion_revision/fixed_cv', 
                        pattern='exp2_fits_baseline_group1_*', full.names=T)
baseline1 = do.call(bind_rows, lapply(paired_fits, read_fit)) %>%
  mutate_at(c('t0','a','md','fd','sz','sd','dpsg','gd',
              'objective','test_objective'), as.numeric) %>%
  mutate(model=str_replace(model, 't0_a_md_fd_sz_sd', 'basemodel'))

paired_fits = list.files(path='./exp2_diffusion_revision/fixed_cv', 
                        pattern='exp2_fits_baseline_group2_*', full.names=T)
baseline2 = do.call(bind_rows, lapply(paired_fits, read_fit)) %>%
  mutate_at(c('t0','a','md','fd','sz','sd','dpsg','gd',
              'objective','test_objective'), as.numeric) %>%
  mutate(model=str_replace(model, 'baseline_group2_', '')) %>%
  mutate(model=str_replace(model, 't0_a_md_fd_sz_sd', 'basemodel'))

xtop = xtop %>% select(fold, md, fd, gd, dpsg) %>% mutate(group='tophalf')
xbot = xbot %>% select(fold, md, fd, gd, dpsg) %>% mutate(group='bottomhalf')
baseline1 = baseline1 %>% select(fold, md, fd, gd, dpsg) %>% mutate(group='baseline1')
baseline2 = baseline2 %>% select(fold, md, fd, gd, dpsg) %>% mutate(group='baseline2')

# ratio
xtop$md/xtop$fd - xbot$md/xbot$fd # -0.1357467
boot_diffs = baseline1$md/baseline1$fd - baseline2$md/baseline2$fd
mean(boot_diffs) # 0.002666068
sort(boot_diffs)[25] # -0.1467574
sort(boot_diffs)[975] # 0.1632713

# gd
xtop$gd - xbot$gd # -0.5795183
boot_diffs = baseline1$gd - baseline2$gd 
mean(boot_diffs) # 0.003557756
sort(boot_diffs)[25] # -0.3003744
sort(boot_diffs)[975] # 0.2943169

# p
xtop$dpsg - xbot$dpsg # -0.03226856
boot_diffs = baseline1$dpsg - baseline2$dpsg 
mean(boot_diffs) # -0.001861636
sort(boot_diffs)[25] # -0.104487
sort(boot_diffs)[975] # 0.0965046
```

```{r, fig.width=6, fig.height=3}
tibble(fold=1:1000,
       ratio=(baseline1$md/baseline1$fd-baseline2$md/baseline2$fd),
       gd=baseline1$gd-baseline2$gd,
       p=baseline1$dpsg-baseline2$dpsg) %>%
  pivot_longer(c('ratio', 'gd', 'p'), names_to='param') %>%
  mutate(param=factor(param, levels=c('ratio','gd','p'))) %>%
  ggplot() +
  geom_histogram(aes(x=value, color='baseline', fill='baseline')) +
  geom_vline(data=tibble(ratio=(xtop$md/xtop$fd-xbot$md/xbot$fd),
                          gd=(xtop$gd-xbot$gd),
                          p=(xtop$dpsg-xbot$dpsg)) %>%
                  pivot_longer(c('ratio', 'gd', 'p'), names_to='param') %>%
                  mutate(param=factor(param, levels=c('ratio','gd','p'))),
             mapping=aes(xintercept=value, color='observed')) +
  geom_vline(data=tibble(ratio=-0.1467574, gd=-0.3003744, p=-0.104487) %>%
                  pivot_longer(c('ratio', 'gd', 'p'), names_to='param') %>%
                  mutate(param=factor(param, levels=c('ratio','gd','p'))),
             mapping=aes(xintercept=value, color='baseline')) +
  geom_vline(data=tibble(ratio=0.1632713, gd=0.2943169, p=0.0965046) %>%
                  pivot_longer(c('ratio', 'gd', 'p'), names_to='param') %>%
                  mutate(param=factor(param, levels=c('ratio','gd','p'))),
             mapping=aes(xintercept=value, color='baseline')) +
  facet_grid(~param) +
  scale_color_manual(values=c(baseline='gray', observed=redblue[1]), name='param difference') +
  scale_fill_manual(values=c(baseline='white', observed='white'), name='param difference')

# ggsave(paste(plotout,'exp2_split_group_param_dist_fitall_1000cv.pdf', sep=''), bg='transparent')
```

# combined figure for IA trials
```{r, fig.width=10, fig.height=3}
( p.ia.main | p.model.comp.full | p.model.params) +
  plot_layout(nrow=1) &
...
# combined figure for ID
```{r, fig.width=10, fig.height=3}
( p.ia.top | p.ia.bot | pweight) +
  plot_layout(nrow=1) &
  plot_annotation(tag_levels=list(c('A','B','C'))) &
  theme(axis.title = element_text(size=10),
        legend.key.size=unit(0.6, 'line'),
...
# ggsave(paste(plotout,'exp2_id.pdf', sep=''), bg='transparent')
```
